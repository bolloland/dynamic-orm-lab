...
class InteractiveRecord
    ...
    def save
        query = "INSERT INTO #{table_name_for_insert} (#{col_names_for_insert})
    ...
    def self.find_by(student_hash)
        value = student_hash.values.first
        # binding.pry
        query = "SELECT * FROM #{self.table_name} WHERE #{student_hash.keys.first} = ?"
      DB[:conn].execute(query, value)
    end
end